...
class AliasFinder:
    ...
    def find_adapter_files_with_aliases(
        ...
        try:
            ...
            print(f"Found {len(adapter_files_with_aliases)} BidAdapter.js files with aliases")
            print("Extracting alias values from each file...")
            
            # Extract aliases from each file
            file_aliases = {}
            files_to_process = adapter_files_with_aliases[:limit] if limit else adapter_files_with_aliases
            
            for i, file_path in enumerate(files_to_process):
                try:
                    aliases = self._extract_aliases_from_file(repo_name, version, file_path)
            ...
            return {
                ...
                "metadata": {
                    ...
                    "files_with_aliases": len([f for f in file_aliases.values() if f]),
                },
            }
            
        except Exception as e:
            raise Exception(f"Error finding adapter files with aliases: {str(e)}") from e
    ...
    def find_adapter_files_with_aliases_batch(
        ...
        try:
            ...
            print(f"Processing {len(adapter_files_with_aliases)} files in batches of {batch_size}")
            print("Extracting alias values from each file...")
            
            # Process files in batches
            file_aliases = {}
            total_batches = (len(adapter_files_with_aliases) + batch_size - 1) // batch_size
            ...
            for batch_num in range(total_batches):
                ...
                for i, file_path in enumerate(batch_files):
                    try:
                        result = self._extract_aliases_from_file(repo_name, version, file_path)
                    ...
                    if request_delay > 0 and i < len(batch_files) - 1:  # Don't delay after last file in batch
    ...
    def _fetch_single_file_content(self, repo_name: str, version: str, file_path: str) -> str:
    ...
    def _parse_aliases_from_content(self, content: str) -> list[str]:
        ...
        for var_name in var_matches:
            ...
            if any(debug_word in var_name.lower() for debug_word in ['debug', 'config', 'param', 'query', 'map']):
                continue
    ...
    def _parse_mixed_array_content(self, array_content: str, aliases: list, full_content: str) -> None:
    ...
    def _remove_js_comments(self, content: str) -> str:
        ...
        content = re.sub(r'(?<!https:)(?<!http:)//.*?$', '', content, flags=re.MULTILINE)
    ...
    def _handle_imported_aliases(self, content: str) -> list[str]:
        ...
        import_pattern = r'import\s*\{([^}]+)\}\s*from\s*[\'"`](\.\./libraries/[^\'"`]+)[\'"`]'
                ...
                try:
                    ...
                    if library_content:
                        for alias_var in alias_vars:
                            library_aliases = self._extract_aliases_from_library(library_content, alias_var)
                            print(f"  Extracted {len(library_aliases)} aliases from {alias_var}")
                            aliases.extend(library_aliases)
                except Exception as e:
                    print(f"  Warning: Could not process import {import_path}: {str(e)}")
    ...
    def _extract_aliases_from_library(self, library_content: str, alias_var_name: str) -> list[str]:
    ...
    def _handle_constant_references(self, content: str) -> list[str]:
        ...
        for const_name in alias_ref_matches:
            ...
            if any(debug_word in const_name.lower() for debug_word in ['debug', 'config', 'param', 'query', 'map', 'string', 'array']):
                continue
